...
function ADODB_logInfo($msg, $file="", $line="") {
	global $log;
	$fMsg = basename($file)."(Line:".$line.") ".$msg;
	if (!empty($log)) {
		$log->debug($fMsg);
	}
}
...
function ADODB_SetDatabaseAdapter(&$db, $index=false, $dbroList=array())
{
	global $_ADODB_ACTIVE_DBS,$_ADODB_ACTIVE_DBROS;
	$obj = null;
	...
	// Set the read-only db
	$dbro_index = array();
	$_ADODB_ACTIVE_DBROS[$index] = array();
	foreach ($dbroList as $dbro) {
		if ($dbro) {
			$_ADODB_ACTIVE_DBROS[$index][] = ADODB_SetDatabaseAdapter($dbro);
		}
	}
...
class ADODB_Active_Record {
	...
	// should be static
	static function SetDatabaseAdapter(&$db, $index=false, $dbroList=array())
	{
		//error_log("Coming into ".self::_pluralize(get_called_class())."'s SetDatabaseAdapter where ".get_class());
		if(!$index || !isset($index)) {
			$index = self::_pluralize(get_called_class());
		}
		return ADODB_SetDatabaseAdapter($db, $index, $dbroList);
	}
	...
	{
	global $ADODB_ASSOC_CASE,$_ADODB_ACTIVE_DBS,$_ADODB_ACTIVE_DBROS;
		...
		if (!$table) { 
			...
			else $table = self::_pluralize(get_class($this));
		}
		$this->foreignName = strtolower(get_class($this)); // CFR: default foreign name
		if ($db) {
			// If this is a slave, use the master db
			$dbSet = false;
			foreach ($_ADODB_ACTIVE_DBROS as $dbindex => $active_dbroList) {
				foreach ($active_dbroList as $active_dbro) {
					if ($_ADODB_ACTIVE_DBS[$active_dbro]->db == $db) {
						$this->_dbat = $dbindex;
						$dbSet = true;
						break;
					}
	...
	function belongsTo($foreignRef,$foreignKey=false, $parentKey='', $parentClass = 'ADODB_Active_Record')
	{
		...
		$ar = new $parentClass(self::_pluralize($foreignRef));
		$ar->foreignName = $foreignRef;
	...
	// update metadata
	function UpdateActiveTableRO($pkeys=false,$forceUpdate=false,$dbatro)
	{
	global $ADODB_ASSOC_CASE,$_ADODB_ACTIVE_DBS , $ADODB_CACHE_DIR, $ADODB_ACTIVE_CACHESECS;
	global $ADODB_ACTIVE_DEFVALS,$ADODB_FETCH_MODE;

		$activedb = $_ADODB_ACTIVE_DBS[$dbatro];
	...
	global $_ADODB_ACTIVE_DBS,$_ADODB_ACTIVE_DBROS;
		...
		if ($readOnly && !empty($_ADODB_ACTIVE_DBROS[$this->_dbat])) {
			// If this is readonly and the Read Only server list is non empty, change to a read only db
			shuffle($_ADODB_ACTIVE_DBROS[$this->_dbat]);
			$activedb = $_ADODB_ACTIVE_DBS[$_ADODB_ACTIVE_DBROS[$this->_dbat][0]];
		} else {
			// No option but to slip into the primary DB
			$activedb = $_ADODB_ACTIVE_DBS[$this->_dbat];
		}
		$db = $activedb->db;
		return $db;
	...
	global $_ADODB_ACTIVE_DBS;
		$activedb = $_ADODB_ACTIVE_DBS[$this->_dbat];
		$table = null;
		if(isset($activedb) && isset($activedb->tables) && isset($activedb->tables[$this->_tableat])) {
	...
	function Reload($forEdit = false)
	{
		$db = $this->DB(); if (!$db) return false;
		$table =& $this->TableInfo();
		$where = $this->GenWhere($db, $table);
		return($this->Load($where, false, $forEdit));
	}
	...
	function Load($where=null,$bindarr=false, $forEdit=false)
	{
		$this->_where = $where;
		...
		return $this->LoadFromRawQuery($qry, $bindarr, $forEdit);
	}

	function LoadFromRawQuery($qry, $bindarr=false, $forEdit=false) {
		global $ADODB_FETCH_MODE;
		
		$db = $this->DB(!$forEdit); if (!$db) return false;
		//ADODB_logInfo("LoadFromRawQuery with ".$db->host.", ".$db->databaseName.", Foredit ".$forEdit);
    ...
    */
	...
	{
		$db = $this->DB(); if (!$db) return false;
		//ADODB_logInfo("Delete ".$this->_table." with ".$db->host);
		$table = $this->TableInfo();
	...
	function Aggregate($function, $column, $whereGroupBy, $bindarr=false) {
		...
		if(!in_array($column, $this->GetAttributeNames()) && $column != "*") {
			throw new InvalidArgumentException("Unknown Column for aggregation $column");
		}
		$db = $this->DB(true); if (!$db || empty($this->_table)) return false;
		//ADODB_logInfo("Aggregate with ".$db->host);
	...
	function CountDistinct($column, $whereGroupBy, $bindarr=false) {
		...
		$db = $this->DB(true); if (!$db || empty($this->_table)) return false;
		//ADODB_logInfo("CountDistinct with ".$db->host);
		return $this->intify(  $db->GetOne("select count(distinct($column)) from ".$this->_table." where ". $whereGroupBy, $bindarr) );
	}

	// returns an array of active record objects
	function Find($whereOrderBy,$bindarr=false,$pkeysArr=false,$extra=array(),$forEdit=false)
	{
		$db = $this->DB(!$forEdit); if (!$db || empty($this->_table)) return false;
		//ADODB_logInfo("Find with ".$db->host);
		$arr = $db->GetActiveRecordsClass(get_class($this),$this->_table, $whereOrderBy,$bindarr,$pkeysArr,$extra);
		return $arr;
	}
	
	// returns an array of active record objects
	function FindFromRawQuery($query,$bindarr=false,$pkeysArr=false,$extra=array(),$forEdit=false)
	{
		$db = $this->DB(!$forEdit); if (!$db || empty($this->_table)) return false;
		//ADODB_logInfo("FindFromRawQuery with ".$db->host);
	...
	global $ADODB_ASSOC_CASE;
		...
		$db = $this->DB(); if (!$db) return false;
		//ADODB_logInfo("Replace with ".$db->host);
		$table = $this->TableInfo();
	...
	{
		$db = $this->DB(); if (!$db) return false;
		//ADODB_logInfo("Update ".$this->_table." with ".$db->host);
		$table = $this->TableInfo();
