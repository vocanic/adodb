				...
				foreach($inputarr as $arr) {
					...
					while(list(, $v) = each($arr)) {
						...
						if ($typ == 'string')
							...
							$arrTmp = array();
							foreach($v as $tmp) {
								$arrElementTyp = gettype($tmp);
								if($arrElementTyp == 'string' ) {
									$arrTmp[] = $this->qstr($tmp);
								} else if ($arrElementTyp == 'double' || $arrElementTyp == 'integer') {
									$arrTmp[] = $tmp;
								}
							}
							$sql .= implode(", ", $arrTmp);
//							error_log("The SQL so far is $sql and arrTmp is ".print_r($arrTmp, true));
//							if($quoteAll) {
//								$sql .= "'".implode("', '", $arrTmp)."'";	
//							} else {
//								$sql .= implode(", ", $arrTmp);
//							}
   ...
   * If $sql == false, then we purge all files in the cache.
	...
	*/
	 ...
	 * @return void
	 */
	function GetActiveRecordsClass2(
			$class, $table, $qry, $bindarr=false, $primkeyArr=false,
			$extra=array(),
			$relations=array())
	{
	global $_ADODB_ACTIVE_DBS;
